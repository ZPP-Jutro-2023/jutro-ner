...
import numpy
from spacy import Language, Vocab, registry
from spacy.pipeline.trainable_pipe import TrainablePipe
from spacy.scorer import PRFScore
from spacy.tokens import Doc
from spacy.training import Example
from thinc.api import Model, Optimizer
from thinc.loss import GradT, GuessT, Loss, LossT, TruthT
...
class RelationExtractor(TrainablePipe):
    ...
    def update(
        ...
        *,
        ...
        updating the pipe's model. Delegates to predict and loss."""
        if losses is None:
            losses = {}
        ...
        loss, gradient = self.loss(truths=truths, guesses=predictions)
        backprop(gradient)
        if sgd is not None:
    ...
    def initialize(
        ...
        *,
        nlp: Language = None,   # pylint: disable=unused-argument
        ...
        if labels is not None:
            ...
            for example in get_examples():
                ...
                for _, label_dict in relations.items():
    ...
    def _examples_to_truth(self, examples: List[Example]) -> Optional[numpy.ndarray]:
        ...
        c = 0
        for _, eg in enumerate(examples):
    ...
    def score(self, examples: Iterable[Example], **_) -> Dict[str, Any]:
...
class MSELoss(Loss):
    def get_loss(self, guesses: GuessT, truths: TruthT) -> LossT:
        loss = ((guesses - truths) ** 2).sum(axis=1).mean()
        return float(loss)
...
@registry.losses("MSELoss.v1")
def configure_MSELoss_v1() -> MSELoss:
    return MSELoss()
